...
SOFTWARE_NAME="petsc"
SOFTWARE_VERSION="${SOFTWARE_VERSION:-3.25.3}"
export SOFTWARE_VERSION
DOCKER_IMAGE="openeuler/petsc"
DOCKER_TAG="${DOCKER_TAG:-3.25.3-oe2403sp3}"
DOCKER_CID=""
SHUNIT2_PATH=""
HAS_PYTHON3=0
...
phase1_install() {
    log "PHASE1" "=== Phase 1: Docker Pull petsc v${SOFTWARE_VERSION} ==="
...
oneTimeSetUp() {
    mkdir -p "${RESULTS_DIR}"
    log "START" "petsc Performance Benchmark v${SOFTWARE_VERSION}"
...
testDockerImageAvailable() {
    ...
    local img="openeuler/petsc:${DOCKER_TAG}"
    local check
...
testSoftwareIsInstalled() {
    ...
    local found=0
    if [ "${HAS_PYTHON3}" -eq 1 ]; then
        docker exec "${DOCKER_CID}" python3 -c "import petsc" 2>/dev/null && found=1
    fi
    if [ "${found}" -eq 0 ]; then
        docker exec "${DOCKER_CID}" which petsc 2>/dev/null && found=1
    fi
    if [ "${found}" -eq 0 ]; then
        docker exec "${DOCKER_CID}" bash -c "ls /opt/petsc*/bin/petsc /usr/local/bin/petsc /usr/bin/petsc 2>/dev/null" | head -1 | grep -q . && found=1
    fi
    if [ "${found}" -eq 0 ]; then
        log "WARN" "petsc not found as Python module or binary, skipping install check"
        startSkipping
        return
    fi
...
petsc Performance Benchmark (shUnit2 + Docker)
Options:
  --check    Check prerequisites only
  -h|--help  Show this help
Environment variables:
  SOFTWARE_VERSION         petsc version (default: 3.25.3)
  DOCKER_TAG              Docker image tag (default: 3.25.3-oe2403sp3)
...
main() {
    ...
    SHUNIT_PARENT="${SCRIPT_DIR}/petsc_test.sh"
    . "${SHUNIT2_PATH}"
}
