...
static BenchResult run_compression_bench(const std::string& input, int level, int iterations) {
    BenchResult r;
    r.data_type = "";
    r.level = level;
    ...
    std::string compressed(dst_cap, '\0');
    size_t comp_size = ZSTD_compress(&compressed[0], dst_cap, input.data(), src_size, level);
    ...
    for (int i = 0; i < iterations; i++) {
        std::string comp_out(dst_cap, '\0');
        auto t0 = std::chrono::high_resolution_clock::now();
        size_t cs = ZSTD_compress(&comp_out[0], dst_cap, input.data(), src_size, level);
        ...
        ZSTD_decompress(&decomp_out[0], input.size(), comp_out.data(), cs);
    ...
    r.compress_speed_mbs = (avg_compress_time > 0) ? input_mb / avg_compress_time : 0.0;
    r.decompress_speed_mbs = (avg_decompress_time > 0) ? input_mb / avg_decompress_time : 0.0;
...
static MicroBlockResult run_block_bench(size_t block_size, int level, int iterations) {
    std::string input = generate_text_data(block_size);
    MicroBlockResult r;
    r.block_size = block_size;
    r.level = level;

    size_t src_size = block_size;
    size_t dst_cap = ZSTD_compressBound(src_size);
    ...
    for (int i = 0; i < iterations; i++) {
        ...
        if (i == 0 && !ZSTD_isError(cs)) compressed_size = cs;
        if (ZSTD_isError(cs)) continue;

        std::string decomp_out(block_size, '\0');
        auto t2 = std::chrono::high_resolution_clock::now();
        ZSTD_decompress(&decomp_out[0], block_size, comp_out.data(), cs);
        auto t3 = std::chrono::high_resolution_clock::now();
    ...
    r.compress_speed_mbs = (avg_comp > 0) ? mb / avg_comp : 0.0;
    r.decompress_speed_mbs = (avg_decomp > 0) ? mb / avg_decomp : 0.0;
    r.compress_latency_us = avg_comp * 1e6;
    r.decompress_latency_us = avg_decomp * 1e6;
    r.compression_ratio = (compressed_size > 0)
        ? static_cast<double>(block_size) / static_cast<double>(compressed_size) : 0.0;
    return r;
}
...
static MtResult run_mt_bench(size_t block_size, int level, int num_threads, int iterations) {
    MtResult r;
    ...
    for (int iter = 0; iter < iterations; iter++) {
        ...
        for (int t = 0; t < num_threads; t++) comp_results[t].resize(dst_cap);
        std::vector<size_t> comp_sizes(num_threads, 0);

        std::vector<std::thread> threads;
        auto c_start = std::chrono::high_resolution_clock::now();
        for (int t = 0; t < num_threads; t++) {
            threads.emplace_back([&, t]() {
                size_t cs = ZSTD_compress(&comp_results[t][0], dst_cap, input.data(), src_size, level);
                if (!ZSTD_isError(cs)) comp_sizes[t] = cs;
            });
        }
        ...
        for (int t = 0; t < num_threads; t++) decomp_results[t].resize(block_size);
        ...
        for (int t = 0; t < num_threads; t++) {
            d_threads.emplace_back([&, t]() {
                if (comp_sizes[t] > 0)
                    ZSTD_decompress(&decomp_results[t][0], block_size, comp_results[t].data(), comp_sizes[t]);
            });
        }
...
int main(int argc, char* argv[]) {
    ...
    std::string version_str = env_version ? env_version : "1.5.6";
    ...
    if (mode == "compression") {
        ...
        for (auto& [name, data] : data_types) {
            for (size_t li = 0; li < NUM_LEVELS; li++) {
                int lvl = LEVELS[li];
                BenchResult r = run_compression_bench(data, lvl, iterations);
                r.data_type = name;
                r.level = lvl;
                r.key = name + "_level" + std::to_string(lvl);
                results.push_back(r);
            }
        }
        ...
        fprintf(fp, "  \"description\": \"Zstd compression/decompression across data patterns and compression levels (1/3/9/19) on ARM64\",\n");
        ...
        fprintf(fp, "    \"compression_levels\": [");
        for (size_t i = 0; i < NUM_LEVELS; i++) {
            fprintf(fp, "%d%s", LEVELS[i], i < NUM_LEVELS - 1 ? ", " : "");
        }
        fprintf(fp, "]\n");
        fprintf(fp, "  },\n");
        ...
        for (int tc : thread_counts) {
            mt_results.push_back(run_mt_bench(65536, 3, tc, iterations));
        }
        ...
        fprintf(fp, "  \"description\": \"Zstd micro benchmarks: block-level compress/decompress latency (level 3) and multithread scaling on ARM64\",\n");
        fprintf(fp, "  \"reference\": \"https://github.com/facebook/zstd\",\n");
        fprintf(fp, "  \"software\": \"zstd\",\n");
        fprintf(fp, "  \"version\": \"%s\",\n", version_str.c_str());
        fprintf(fp, "  \"architecture\": \"arm64\",\n");
