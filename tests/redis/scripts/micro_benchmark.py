...
def run_micro_benchmark(benchmark_bin, output_file, num_ops, value_size, iterations, port):
...
def main():
    if len(sys.argv) < 3:
        print("Usage: micro_benchmark.py <benchmark_bin> <output_file> "
    ...
    success = run_micro_benchmark(benchmark_bin, output_file, num_ops,
    ...
    if success and os.path.exists(output_file):
        try:
            ...
            if "single_commands" in results:
                print(f"[BENCHMARK_MICRO] Commands: {list(results['single_commands'].keys())}")
            if "pipeline" in results:
                print(f"[BENCHMARK_MICRO] Pipeline sizes: {list(results['pipeline'].keys())}")
            if "multithread_scaling" in results:
                print(f"[BENCHMARK_MICRO] Client counts: {list(results['multithread_scaling'].keys())}")
