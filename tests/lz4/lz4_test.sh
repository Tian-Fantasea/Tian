...
phase1_build() {
    ...
    if [ -f "/usr/local/lib/liblz4.so" ] || [ -f "/usr/local/lib/liblz4.a" ] || \
       [ -f "/usr/lib/liblz4.so" ] || [ -f "/usr/lib/liblz4.a" ]; then
        log "PHASE1" "LZ4 library found, skipping build (will recompile benchmark only)"
    fi

    create_build_tmpdir

    local LZ4_SRC_DIR="${BUILD_TMPDIR}/lz4_src"
    local LZ4_BUILD_DIR="${BUILD_TMPDIR}/build"
    ...
    log "PHASE1" "Cloning LZ4 tag ${ver_tag}..."
    git clone --branch "${ver_tag}" --depth 1 \
        https://github.com/lz4/lz4.git \
        "${LZ4_SRC_DIR}" 2>&1 | tee -a "${LOG_FILE}" || {
        log "ERROR" "Failed to clone LZ4 tag ${ver_tag} (verify SOFTWARE_VERSION=${SOFTWARE_VERSION} is a valid release tag)"
        return 1
    }

    local cmake_src_dir="${LZ4_SRC_DIR}/build/cmake"
    if [ ! -d "${cmake_src_dir}" ]; then
        log "WARN" "build/cmake not found, falling back to repo root"
        cmake_src_dir="${LZ4_SRC_DIR}"
    fi

    log "PHASE1" "Configuring CMake..."
    mkdir -p "${LZ4_BUILD_DIR}"
    (cd "${LZ4_BUILD_DIR}" && cmake \
        -DCMAKE_BUILD_TYPE=Release \
        -DCMAKE_INSTALL_PREFIX="${LZ4_INSTALL_DIR}" \
        -DBUILD_STATIC_LIBS=ON \
        "${cmake_src_dir}" 2>&1 | tee -a "${LOG_FILE}") || {
        log "WARN" "BUILD_STATIC_LIBS option unsupported, retrying without it..."
        (cd "${LZ4_BUILD_DIR}" && cmake \
            -DCMAKE_BUILD_TYPE=Release \
            -DCMAKE_INSTALL_PREFIX="${LZ4_INSTALL_DIR}" \
            "${cmake_src_dir}" 2>&1 | tee -a "${LOG_FILE}") || {
    ...
    log "PHASE1" "Compiling LZ4..."
    (cd "${LZ4_BUILD_DIR}" && make -j$(nproc) 2>&1 | tee -a "${LOG_FILE}") || {
        log "ERROR" "LZ4 compilation failed"
        return 1
    }

    log "PHASE1" "Installing LZ4..."
    (cd "${LZ4_BUILD_DIR}" && make install 2>&1 | tee -a "${LOG_FILE}") || {
...
Environment variables:
  SOFTWARE_VERSION          LZ4 version (default: 1.9.4; supported: 1.9.4, 1.10.0)
...
Examples:
  ...
  # LZ4 1.10.0 (latest, multithread edition)
  SOFTWARE_VERSION=1.10.0 ./lz4_test.sh
  # LZ4 1.9.4 (stable, ARM64 decode +20%)
