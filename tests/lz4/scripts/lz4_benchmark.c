...
static std::string generate_text_data(size_t size) {
    std::string words[] = {"hello", "world", "test", "benchmark", "lz4",
...
static BenchResult run_compression_bench(const std::string& input, int iterations) {
    ...
    int max_comp_size = LZ4_compressBound(static_cast<int>(input.size()));
    std::string compressed(max_comp_size, '\0');

    int comp_size = LZ4_compress_default(input.data(), compressed.data(),
                                          static_cast<int>(input.size()), max_comp_size);
    ...
    for (int i = 0; i < iterations; i++) {
        std::string comp_out(max_comp_size, '\0');
        auto t0 = std::chrono::high_resolution_clock::now();
        int cs = LZ4_compress_default(input.data(), comp_out.data(),
                                       static_cast<int>(input.size()), max_comp_size);
        ...
        LZ4_decompress_safe(comp_out.data(), decomp_out.data(), cs,
...
static std::vector<AccelResult> run_acceleration_sweep(const std::string& input,
                                                         const std::vector<int>& accel_values,
                                                         int iterations) {
    std::vector<AccelResult> results;
    int max_comp_size = LZ4_compressBound(static_cast<int>(input.size()));

    for (int accel : accel_values) {
        AccelResult r;
        ...
        for (int i = 0; i < iterations; i++) {
            std::string comp_out(max_comp_size, '\0');
            auto t0 = std::chrono::high_resolution_clock::now();
            int cs = LZ4_compress_fast(input.data(), comp_out.data(),
                                        static_cast<int>(input.size()), max_comp_size, accel);
            auto t1 = std::chrono::high_resolution_clock::now();
        ...
        double input_mb = static_cast<double>(input.size()) / (1024.0 * 1024.0);
        r.compress_speed_mbs = input_mb / avg_time;
...
static MicroBlockResult run_block_bench(size_t block_size, int iterations) {
    ...
    int max_comp_size = LZ4_compressBound(static_cast<int>(block_size));
    ...
    for (int i = 0; i < iterations; i++) {
        ...
        auto t0 = std::chrono::high_resolution_clock::now();
        int cs = LZ4_compress_default(input.data(), comp_out.data(),
                                       static_cast<int>(block_size), max_comp_size);
        auto t1 = std::chrono::high_resolution_clock::now();
...
static MtResult run_mt_bench(size_t block_size, int num_threads, int iterations) {
    ...
    std::string input = generate_text_data(block_size);
    int max_comp_size = LZ4_compressBound(static_cast<int>(block_size));
    int input_size = static_cast<int>(block_size);
    ...
    for (int iter = 0; iter < iterations; iter++) {
        ...
        std::vector<int> comp_sizes(num_threads);

        auto c_start = std::chrono::high_resolution_clock::now();
        std::vector<std::thread> threads;
        for (int t = 0; t < num_threads; t++) {
            threads.emplace_back([&, t]() {
                comp_results[t].resize(max_comp_size);
                comp_sizes[t] = LZ4_compress_default(input.data(), comp_results[t].data(),
        ...
        for (int t = 0; t < num_threads; t++) {
            d_threads.emplace_back([&, t]() {
                decomp_results[t].resize(input_size);
                LZ4_decompress_safe(comp_results[t].data(), decomp_results[t].data(),
                                    comp_sizes[t], input_size);
            });
        }
...
int main(int argc, char* argv[]) {
    ...
    if (mode == "compression") {
        ...
        std::vector<int> accel_values = {1, 2, 5, 10, 20, 50, 100, 500, 1000};
        std::vector<AccelResult> accel_results = run_acceleration_sweep(text_input, accel_values, iterations);
        ...
        fprintf(fp, "  },\n");
        fprintf(fp, "  \"acceleration_sweep\": {\n");
        for (size_t i = 0; i < accel_results.size(); i++) {
            auto& r = accel_results[i];
            fprintf(fp, "    \"%d\": {\n", r.acceleration);
            fprintf(fp, "      \"compress_speed_mbs\": %.2f,\n", r.compress_speed_mbs);
            fprintf(fp, "      \"compression_ratio\": %.4f,\n", r.compression_ratio);
            fprintf(fp, "      \"compressed_size_bytes\": %zu\n", r.compressed_size);
            fprintf(fp, "    }%s\n", i < accel_results.size() - 1 ? "," : "");
        }
        fprintf(fp, "  }\n");
        fprintf(fp, "}\n");
        fclose(fp);
