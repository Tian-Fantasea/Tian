...
_mysql_cmd() {
    if [ -n "${OB_PASSWORD:-}" ]; then
        echo "mysql -h${OB_HOST:-127.0.0.1} -P${OB_PORT:-2881} -u${OB_USER:-root@test} -p'${OB_PASSWORD}'"
    else
        echo "mysql -h${OB_HOST:-127.0.0.1} -P${OB_PORT:-2881} -u${OB_USER:-root@test}"
    fi
}
...
run_benchmark_stress() {
    ...
    if eval "$(_mysql_cmd)" -e "SELECT 1" 2>/dev/null; then
        ...
        while [ "$(date +%s)" -lt "$((${start_time} + ${stress_duration}))" ]; do
            for t in $(seq 1 "${stress_threads}"); do
                eval "$(_mysql_cmd)" -e "SELECT 1 FROM dual" 2>/dev/null && total_ops=$((total_ops + 1))
            done
        done
        local elapsed
...
Examples:
  ...
  Custom warehouses:  ./oceanbase_arm64_perf_workflow.sh -w 100 --terminals 50 -i 5
EOF
}
