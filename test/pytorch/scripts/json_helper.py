...
def cmd_write_version_info(args):
    ...
    d = {
        ...
        "python_version": v[9],
        "pytorch_version": v[10],
        "numpy_version": v[11],
        "cuda_available": int(v[12]) if v[12] not in ("unknown", "") else 0,
        "torch_num_threads": int(v[13]) if v[13] not in ("unknown", "") else 0,
        "has_compile": int(v[14]) if v[14] not in ("unknown", "") else 0,
        "neon_available": v[15] if len(v) > 15 else "0",
    }
    arm64 = {
        "is_arm64": d["architecture"] in ("aarch64", "arm64"),
        "neon_available": d["neon_available"] not in ("0", "unknown"),
        "cuda_available": bool(d["cuda_available"]),
    }
