...
static std::string generate_text_data(size_t size) {
    std::string words[] = {"hello", "world", "test", "benchmark", "zstd",
...
static BenchResult run_compression_bench(const std::string& input, int compression_level, int iterations) {
    BenchResult r;
    r.original_size = input.size();
    size_t comp_bound = ZSTD_compressBound(input.size());
    std::string compressed(comp_bound, '\0');

    size_t comp_size = ZSTD_compress(compressed.data(), comp_bound,
    ...
    if (ZSTD_isError(comp_size)) {
        ...
        r.compress_speed_mbs = 0;
        r.decompress_speed_mbs = 0;
        r.compress_latency_us = 0;
        r.decompress_latency_us = 0;
        return r;
    }

    r.compressed_size = comp_size;
    r.compression_ratio = static_cast<double>(input.size()) / static_cast<double>(comp_size);
    ...
    for (int i = 0; i < iterations; i++) {
        std::string comp_out(comp_bound, '\0');
        auto t0 = std::chrono::high_resolution_clock::now();
        size_t cs = ZSTD_compress(comp_out.data(), comp_bound,
                                   input.data(), input.size(), compression_level);
        auto t1 = std::chrono::high_resolution_clock::now();
        total_compress_time += std::chrono::duration<double>(t1 - t0).count();

        std::string decomp_out(input.size(), '\0');
        auto t2 = std::chrono::high_resolution_clock::now();
        size_t ds = ZSTD_decompress(decomp_out.data(), input.size(),
                                     comp_out.data(), cs);
...
static std::vector<LevelResult> run_level_sweep(const std::string& input,
    ...
    for (int level : levels) {
        ...
        for (int i = 0; i < iterations; i++) {
            ...
            if (i == 0) first_comp_size = cs;

            std::string decomp_out(input.size(), '\0');
            auto t2 = std::chrono::high_resolution_clock::now();
            ZSTD_decompress(decomp_out.data(), input.size(), comp_out.data(), cs);
            auto t3 = std::chrono::high_resolution_clock::now();
            total_decomp += std::chrono::duration<double>(t3 - t2).count();
        }

        double avg_comp = total_comp / iterations;
        double avg_decomp = total_decomp / iterations;
        double input_mb = static_cast<double>(input.size()) / (1024.0 * 1024.0);
        r.compress_speed_mbs = input_mb / avg_comp;
        r.decompress_speed_mbs = input_mb / avg_decomp;
        r.compressed_size = first_comp_size;
        r.compression_ratio = static_cast<double>(input.size()) / static_cast<double>(first_comp_size);
        r.compress_latency_us = avg_comp * 1e6;
        results.push_back(r);
    }
    return results;
}
...
static MicroBlockResult run_block_bench(size_t block_size, int compression_level, int iterations) {
    std::string input = generate_text_data(block_size);
    MicroBlockResult r;
    r.block_size = block_size;
    size_t comp_bound = ZSTD_compressBound(block_size);
    ...
    for (int i = 0; i < iterations; i++) {
        ...
        auto t0 = std::chrono::high_resolution_clock::now();
        size_t cs = ZSTD_compress(comp_out.data(), comp_bound,
                                   input.data(), block_size, compression_level);
        auto t1 = std::chrono::high_resolution_clock::now();
        total_comp += std::chrono::duration<double>(t1 - t0).count();
        if (i == 0) compressed_size = cs;

        std::string decomp_out(block_size, '\0');
        auto t2 = std::chrono::high_resolution_clock::now();
        ZSTD_decompress(decomp_out.data(), block_size, comp_out.data(), cs);
        auto t3 = std::chrono::high_resolution_clock::now();
        total_decomp += std::chrono::duration<double>(t3 - t2).count();
    }
...
static MtResult run_mt_bench(size_t block_size, int num_threads, int compression_level, int iterations) {
    MtResult r;
    ...
    for (int iter = 0; iter < iterations; iter++) {
        ...
        std::vector<size_t> comp_sizes(num_threads);

        auto c_start = std::chrono::high_resolution_clock::now();
        std::vector<std::thread> threads;
        for (int t = 0; t < num_threads; t++) {
            threads.emplace_back([&, t]() {
                comp_results[t].resize(comp_bound);
                comp_sizes[t] = ZSTD_compress(comp_results[t].data(), comp_bound,
                                               input.data(), block_size, compression_level);
                comp_results[t].resize(comp_sizes[t]);
            });
        }
        ...
        for (int t = 0; t < num_threads; t++) {
            d_threads.emplace_back([&, t]() {
                decomp_results[t].resize(block_size);
                ZSTD_decompress(decomp_results[t].data(), block_size,
                                comp_results[t].data(), comp_sizes[t]);
            });
        }
...
int main(int argc, char* argv[]) {
    if (argc < 4) {
        ...
        fprintf(stderr, "  mode: compression | micro\n");
        return 1;
    }
    ...
    if (mode == "compression") {
        ...
        for (auto& [name, data] : data_types) {
            BenchResult r = run_compression_bench(data, 3, iterations);
            r.data_type = name;
            results.push_back(r);
        }

        std::string text_input = generate_text_data(data_size);
        std::vector<int> levels = {1, 2, 3, 5, 7, 9, 12, 15, 19, 22};
        std::vector<LevelResult> level_results = run_level_sweep(text_input, levels, iterations);
        ...
        fprintf(fp, "    \"compress_speed_mbs\": {\"unit\": \"MB/s\", \"description\": \"Compression throughput\"},\n");
        fprintf(fp, "    \"decompress_speed_mbs\": {\"unit\": \"MB/s\", \"description\": \"Decompression throughput\"},\n");
        fprintf(fp, "    \"compression_ratio\": {\"unit\": \"ratio\", \"description\": \"Original/compressed size ratio\"}\n");
        fprintf(fp, "  },\n");
        fprintf(fp, "  \"parameters\": {\n");
        fprintf(fp, "    \"data_size_bytes\": %zu,\n", data_size);
        fprintf(fp, "    \"default_compression_level\": 3,\n");
        ...
        fprintf(fp, "  },\n");
        fprintf(fp, "  \"level_sweep\": {\n");
        for (size_t i = 0; i < level_results.size(); i++) {
            auto& r = level_results[i];
            fprintf(fp, "    \"%d\": {\n", r.level);
            fprintf(fp, "      \"compress_speed_mbs\": %.2f,\n", r.compress_speed_mbs);
            fprintf(fp, "      \"decompress_speed_mbs\": %.2f,\n", r.decompress_speed_mbs);
            fprintf(fp, "      \"compression_ratio\": %.4f,\n", r.compression_ratio);
        ...
        for (auto bs : block_sizes) {
            block_results.push_back(run_block_bench(bs, 3, iterations));
        }
        ...
        for (int tc : thread_counts) {
            mt_results.push_back(run_mt_bench(65536, tc, 3, iterations));
        }
        ...
        fprintf(fp, "],\n");
        fprintf(fp, "    \"compression_level\": 3,\n");
        fprintf(fp, "    \"iterations\": %d,\n", iterations);
