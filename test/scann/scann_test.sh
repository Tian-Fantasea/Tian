...
K="${K:-10}"
MIN_QPS_BRUTE="${MIN_QPS_BRUTE:-10}"
MAX_LATENCY_SINGLE_US="${MAX_LATENCY_SINGLE_US:-5000}"
MIN_RECALL_BRUTE="${MIN_RECALL_BRUTE:-0.50}"
...
check_prerequisites() {
    ...
    if ! python3 -c "import scann" 2>/dev/null; then
        log "ERROR" "scann Python module is not installed. Please install scann-cpu"
        log "ERROR" "  Recommended: pip install scann==${SOFTWARE_VERSION}"
        errors=$((errors + 1))
    else
        log "CHECK" "ScaNN OK: $(python3 -c 'import scann; print(scann.__version__)' 2>&1 | head -1)"
    fi
...
phase2_verify() {
    ...
    local timestamp arch kernel os_name cpu_model cores mem_mb python_ver scann_ver numpy_ver neon cores_par
    ...
    scann_ver="$(python3 -c 'import scann; print(scann.__version__)' 2>/dev/null | tr -d '\n\t' || echo 'unknown')"
    numpy_ver="$(python3 -c 'import numpy; print(numpy.__version__)' 2>/dev/null | tr -d '\n\t' || echo 'unknown')"
    neon="$(grep -c 'neon\|asimd' /proc/cpuinfo 2>/dev/null | tr -d '\n\t' || echo '0')"
    cores_par="${cores}"
    python3 "${JSON_HELPER}" "${RESULTS_DIR}/version_info.json" write_version_info \
        ...
        "${python_ver}" "${scann_ver}" "${numpy_ver}" "${neon}" "${cores_par}"
...
testSoftwareIsInstalled() {
    local found=0
    if python3 -c "import scann" 2>/dev/null; then found=1; fi
    if [ "${found}" -eq 0 ]; then
        echo "WARNING: ScaNN not installed, skipping install check"
        startSkipping
        return
    fi
    assertTrue "ScaNN Python module should be importable" "[ ${found} -eq 1 ]"
}

testSoftwareVersionMatches() {
    local ver="unknown"
    ver="$(python3 -c 'import scann; print(scann.__version__)' 2>/dev/null | tr -d '\n\t' || echo 'unknown')"
    if [ "${ver}" = "unknown" ]; then
        startSkipping
        return
    fi
...
np.random.seed(42)
data = np.random.random((100, 10)).astype('float32')
searcher = scann.scann_ops_pybind.builder(data, 5, 'squared_l2').score_brute_force().build()
neighbors, distances = searcher.search(data[:1], final_num_neighbors=5)
print('OK')
" 2>&1)"
    assertTrue "ScaNN basic search should succeed" "[ $? -eq 0 ]"
}
...
testBenchmarkPrimaryThroughputAboveThreshold() {
    ...
    local brute_qps
    brute_qps="$(json_get "${bench_file}" results_summary BruteForce_L2 qps)"
    if [ "${brute_qps}" = "NULL" ] || [ -z "${brute_qps}" ]; then
        local count
        count="$(json_count_results "${bench_file}")"
        if [ "${count}" -gt 0 ]; then
            brute_qps="$(json_avg_throughput "${bench_file}" results_summary BruteForce_L2 qps)"
        else
            startSkipping
            return
        fi
    fi
    echo "[DIAG] BruteForce QPS: ${brute_qps} (threshold: ${MIN_QPS_BRUTE})"
    assertTrue "BruteForce QPS (${brute_qps}) should be >= ${MIN_QPS_BRUTE}" \
        "[ $(echo "${brute_qps} >= ${MIN_QPS_BRUTE}" | bc -l) -eq 1 ]"
}
...
testBenchmarkMicroSearchLatencyBelowThreshold() {
    ...
    local latency
    latency="$(json_max_latency "${bench_file}" results search_latency_by_batch avg_latency_per_query_us)"
    echo "[DIAG] Micro search latency: ${latency} us (threshold: ${MAX_LATENCY_SINGLE_US})"
    assertTrue "Micro search latency (${latency}us) should be <= ${MAX_LATENCY_SINGLE_US}us" \
        "[ $(echo "${latency} <= ${MAX_LATENCY_SINGLE_US}" | bc -l) -eq 1 ]"
}

testBenchmarkMicroHasBuildTimeData() {
    local bench_file="${RESULTS_DIR}/micro_benchmark.json"
    if [ ! -f "${bench_file}" ]; then
        startSkipping
        return
    ...
    local has_build
    has_build="$(json_contains "${bench_file}" build_times)"
    assertTrue "Micro benchmark should have build_times data" "[ ${has_build} -eq 1 ]"
}
...
ScaNN ARM64 Performance Benchmark (shUnit2)
Options:
  --check    Check prerequisites only (do not run benchmarks)
  -h|--help  Show this help
Environment variables:
  SOFTWARE_VERSION   ScaNN version (default: 1.4.2)
  ...
  MIN_QPS_BRUTE      Minimum BruteForce QPS threshold (default: 10)
  MAX_LATENCY_SINGLE_US Maximum latency threshold (default: 5000)
  MIN_RECALL_BRUTE   Minimum BruteForce recall threshold (default: 0.5)
Examples:
  # Check prerequisites
  ./scann_test.sh --check
  # Full run
  ./scann_test.sh
  # Custom params
  DATA_SCALE=100K DATA_DIM=128 ITERATIONS=1 ./scann_test.sh
USAGE
}
