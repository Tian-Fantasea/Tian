...
def cmd_throughput_ge(data, threshold, keys):
    keys_list = keys.split() if isinstance(keys, str) else keys
    val = navigate(data, keys_list)
    if val is None:
        print(0)
        return
...
def cmd_write_version_info(data, args):
    ...
    python_version = args[9] if len(args) > 9 else ""
    scann_version = args[10] if len(args) > 10 else ""
    numpy_version = args[11] if len(args) > 11 else ""
    neon_support = args[12] if len(args) > 12 else "0"
    parallelism = args[13] if len(args) > 13 else "4"
    ...
    version_info = {
        ...
        "neon_support": neon_support,
        "parallelism": int(parallelism) if parallelism.isdigit() else parallelism,
    }
