...
def main():
    ...
    if vi:
        ...
        lines.append(f"hnswlib Version: {vi.get('hnswlib_version', 'N/A')}")
        lines.append(f"Python:         {vi.get('python_version', 'N/A')}")
        lines.append(f"NumPy:          {vi.get('numpy_version', 'N/A')}")
        ip = vi.get('index_properties', {})
        if ip:
            lines.append(f"Index defaults: M={ip.get('M', 'N/A')}, ef_construction={ip.get('ef_construction', 'N/A')}")
        lines.append("")
    ...
    if ann:
        ...
        lines.append(f"Dataset: {params.get('num_vectors', 'N/A')} vectors, {params.get('dimension', 'N/A')} dims, k={params.get('k', 'N/A')}")
    ...
    if micro:
        ...
        for op_name, res in results.items():
            lines.append(f"  {op_name}: {json.dumps(res, indent=2)}")
        lines.append("")
