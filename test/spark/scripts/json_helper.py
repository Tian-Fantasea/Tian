...
def cmd_write_version_info(filepath, timestamp, arch, kernel, os_name, cpu_model,
                           cores, mem_mb, sw_name, sw_version, java_ver,
                           spark_home, parallelism, scala_ver, python_ver):
    data = {
    ...
    for ip in input_paths:
        if os.path.exists(ip):
            ...
            if isinstance(data, dict):
                ...
                if bench_name == "TPC-DS":
                    merged["primary_benchmark"] = data
                elif bench_name == "Structured Streaming":
                    merged["secondary_benchmark"] = data
                elif bench_name == "Micro-Benchmarks":
                    merged["micro_benchmark"] = data
                elif bench_name == "MLlib":
                    if not merged.get("mllib_benchmark"):
                        merged["mllib_benchmark"] = data
