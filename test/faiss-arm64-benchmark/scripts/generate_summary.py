...
def main():
    ...
    lines = []
    lines.append("=" * 70)
    lines.append("Faiss ARM64 Performance Benchmark Summary")
    lines.append("=" * 70)
    ...
    if vi:
        ...
        lines.append(f"Faiss Version:  {vi.get('faiss_version', 'N/A')}")
    ...
    if ann:
        ...
        for index_name, res in results.items():
            if "error" in res:
                ...
                lines.append(f"    Index size:       {res.get('index_size_bytes', 'N/A')} bytes")
        lines.append("")
